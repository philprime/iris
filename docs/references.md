...
- **Pinned tooling via the `go.mod` `tool` directive** (controller-gen, setup-envtest, …) — the
  philprime house style; equivalent to flux's `go-install-tool` macro but matches the other repos.
- **Dependency-light reconcile** — adopt source-controller's patch-on-defer _pattern_ using
